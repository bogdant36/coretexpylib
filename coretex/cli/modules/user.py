...
from ..config.user import UserConfiguration
...
def userConfigurationPrompt(retryCount: int = 0) -> UserConfiguration:
    if retryCount >= 3:
    ...
    if response.hasFailed():
        ...
        return userConfigurationPrompt(retryCount + 1)
...
def initializeUserSession() -> None:
    try:
        config = UserConfiguration.initialize()
    except:
        config = userConfigurationPrompt()
